    ...
    with open(filename, "w") as w:
        ...
        while curModisStep.getDateTimeStart() < end_date:
            ...
            if html:
                w.write(
                    "    <td>&lt;YYYY&gt;{}</td>\n".format(
                        export_octad.getDateTimeStart().strftime("%j")
                    )
                )

                w.write(
                    "    <td>download -b/e: {}<br>window -b/e:<ul>".format(
                ...
                w.write(
                    ...
                    "  -b {1} -e {1} M?D13A2\n".format(
                        b_count, curModisStep.getDateTimeStart().strftime("%Y-%m-%d")
                    )
                )
                w.write(
                    "python modis_collect.py --interleave --cleanup --last-collected {} .\n".format(
            ...
            while nexports <= cs:
                ...
                if html:
                    w.write(
                        "<li>&lt;YYYY{}&gt-{}</li>".format(
                            "" if exp_date.year == curModisStep.Year else "-1",
                            exp_date.strftime("%m-%d"),
                        )
                    )
                else:
                    w.write(
                        "python modis_window.py -b {0} -e {0} --clip-valid --round-int 2 \\\n"
...
def build_calendar(begin_date, end_date, filename):
    ...
    with open(filename, "w") as w:
        ...
        while expectedRelease < end_date:
            ...
            while nexports <= 6:
                latency = int(
                    (expectedRelease - export_dekad.getDateTimeEnd()) / datetime.timedelta(days=1)
                )
                stages[
                    "{}-d{}".format(months[export_dekad.Month - 1], export_dekad.SliceInMonth)
                ] = "{};{}".format(nexports - 1, latency)
            ...
            while iterDekad.getDateTimeStart() < end_date:
                w.write(
                    ";{}".format(
                        stages.get(
                            "{}-d{}".format(months[iterDekad.Month - 1], iterDekad.SliceInMonth),
                            ";",
                        )
                    )
                )

...
def curate_downloads(folder, tiles, begin_date, end_date, allow_missing=0) -> list[str]:
    """
    ...
    :return: list of files being kept
    """
    curated: list[str] = []
    re_hdf_slice_date = re.compile(r"^.+A(\d{7})\.h\d{2}v\d{2}\.\d{3}\.\d{13}\.h(df|5)$")
    re_hdf_production_date = re.compile(r"^.+(\d{13})\.h(df|5)$")
    re_hdf_tile = re.compile(r"^.+(h\d{2}v\d{2})\.\d{3}\.\d{13}\.h(df|5)$")
    ...
    if len(hdf_files) < 1:
        hdf_files = [x.as_posix() for x in Path(folder).glob("*.h5")]
    if len(hdf_files) < 1:
        return False
    ...
    hdf_dates = set(
        filter(
            lambda dte: re.compile(r"^\d{7}$").match(dte),
            [re.sub(re_hdf_slice_date, r"\1", x) for x in hdf_files],
        )
    )

    for sDte in sorted(hdf_dates):
        re_cur_dte = re.compile(r"^.+A" + sDte + r"\.h\d{2}v\d{2}\.\d{3}\.\d{13}\.h(df|5)$")
        ...
        for file in filter(lambda f: re_cur_dte.match(f), hdf_files):
            if sDte not in time_slices:
                ...
                if hdfTile not in tile2file:
                    print(
                        "We do not want this tile: {}; removing this file: {}".format(hdfTile, file)
                    )
                    os.remove(file)
                else:
                    if len(tile2file[hdfTile]) == 0:
                    ...
                    elif int(re.sub(re_hdf_production_date, r"\1", file)) > int(
                        re.sub(re_hdf_production_date, r"\1", tile2file[hdfTile])
                    ):
                        ...
                        os.remove(file)
        curated.extend(tile2file.values())
